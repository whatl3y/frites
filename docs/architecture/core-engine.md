...
- **Empty diff** — synthesizer touched no files: candidate status becomes `empty`, `usable` is false, fallback with "produced no usable change".
- **Errored / timed-out** — the synthesizer process fails or is reaped by the idle/hard timeout: not usable, fallback. The synthesizer ships a concrete `synthesisHardTimeoutMs` (default 30 min) ceiling unlike the off-by-default per-child hard timeout.
- **Oracle fail** — the synthesis runs and produces a diff but fails the SAME build/lint/test oracle: fallback with "failed the oracle".
- **Over-broad** — synthesis passes but exceeds the `synthesisMaxBlastFactor ×` ceiling: fallback to avoid an over-broad change.
