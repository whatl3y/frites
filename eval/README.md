# distrai evaluation & benchmarking

Two harnesses live here:

- **`value-gate.ts`** (`pnpm eval`) — distrai-specific A/B: does fan-out beat a single agent on a
...
- **`bench-matrix.ts`** (`pnpm bench`) — runs a *standard* agentic-coding harness against many distrai
  configs **and** raw-model baselines on the **same** tasks, then tables accuracy + cost + latency so
  you can compare distrai to normal models. This doc is its runbook.

## Results

distrai is a **fusion panel**: it runs several coding agents and synthesizes one answer. The rows
...
> _Benchmark: Aider polyglot (Exercism exercises across 6 languages). Backend: distrai child agents
...
3. **Model diversity** — distrai's child kinds are `claude-cli` and `codex-cli` today, so it fuses
...
## How distrai connects to a harness

distrai already *is* a model-provider endpoint, so harnesses point at it the way they'd point at a
...
`ANTHROPIC_API_BASE` (LiteLLM / Aider) pointing at the gateway, plus `ANTHROPIC_API_KEY=distrai`
(auth is off by default — see [apps/gateway/src/index.ts](../apps/gateway/src/index.ts) `authorized()`).

### Five facts about distrai that shape the setup (verified in the gateway source)

1. **Child model is chosen by config, not by the request.** `model = body.model ?? "distrai"` is a
...
3. **A `model` containing `haiku`/`small`/`fast` short-circuits the council** to a single cheap agent
   (`isBackgroundModel`). The matrix uses `distrai-council` for distrai rows to avoid this.
4. **`maxTurns` defaults to 60 and force-stops a session.** Long tool-loop tasks would hit it and
   score an artificial failure, so distrai conditions set `maxTurns: 200`.
...
Why first: text-edit based (no tool-loop — distrai just returns formatted edits), and its public
leaderboard gives you raw-Opus/raw-GPT and published baselines for free while you run the same 225
problems locally for the distrai conditions.
...
**Architecture (why two extra env vars):** the distrai gateway runs on your **host** (its child
...
```bash
export DISTRAI_BENCH_GATEWAY_HOST=0.0.0.0    # required for Docker mode (distrai conditions)
```
...
by default, so set `DISTRAI_GATEWAY_TOKEN` if that matters on your network. For the `raw-opus` /
...
**Watch `well%` (percent well-formed).** distrai's synthesizer must preserve aider's edit format; if
`well%` is low the pass-rate is capped by formatting, not reasoning — that's a distrai synthesis fix,
...
- **Auth** (`--auth`): `oauth` (children use your host claude/codex subscription — no API key) vs
  ...
  quality one — but it's the only way to get real per-token $ for distrai (codex only reports cost on
...
> The adapter writes a temp `--read-model-settings` YAML aliasing `anthropic/distrai-council` so
...
integrity), `tok` (prompt+completion — the comparable usage metric, since distrai cost reads $0 under
OAuth/subscription), `cost($)` (gateway-measured spend for distrai rows; aider's own number for raw
rows), `dur(s)` (wall-clock per condition).

**Cost attribution (`--price`):** distrai's `cost($)` sums each child's spend from the gateway log.
...
[bench-matrix.ts](bench-matrix.ts), or via env (`DISTRAI_BENCH_CODEX_MODEL`, `DISTRAI_BENCH_CODEX_IN`,
`DISTRAI_BENCH_CODEX_OUT`) — the shipped rates are placeholders. ⚠️ `--price` **pins codex to that
...
it ships a `swe_bench` task + agent scaffold, and the tool-loop runs against distrai's Messages
surface (fact #2). It's Docker-per-task over 500 tasks, so **subsample**. This is where the
`fanOutScope` toggle (`distrai-council-firstturn` vs `distrai-council-perturn`) actually diverges:
...
To wire it into `bench-matrix.ts`, point `DISTRAI_BENCH_HARNESS` at an Inspect adapter that reads the
same env contract (`DISTRAI_BENCH_URL`, `DISTRAI_BENCH_MODEL`, `DISTRAI_BENCH_RESULT`) and emits the
...
`bench-matrix.ts` invokes `DISTRAI_BENCH_HARNESS` (default `harness/aider-polyglot.sh`) once per
condition with the gateway already up, and reads back a JSON file at `$DISTRAI_BENCH_RESULT`:

```json
{"pass_rate_1": 0, "pass_rate_2": 0, "percent_well_formed": 0, "cost_usd": 0, "n": 0, "notes": "..."}
```

Env the harness receives: `DISTRAI_BENCH_URL` (empty for passthrough baselines), `DISTRAI_BENCH_MODEL`,
`DISTRAI_BENCH_RESULT`, `DISTRAI_BENCH_NUM_TESTS`, `DISTRAI_BENCH_CONDITION`, and — for distrai
